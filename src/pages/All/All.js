...
const All = (props) => {
  ...
  const getAll = async () => {
    let response = await fetch(`https://uselessapi.com/u-c/all-JSON`);
        ...
        <Col>
          <Suspense fallback={<div>Fetching latest UseLess routes...</div>}>
            {routes.map((routes) => (
              <Card>
                <CardBody>
                  <NavLink href={"https://uselessapi.com/u-c-r" + routes.route}>
                    {" "}
                    <CardTitle tag="h4">
                  ...
                  <a href={"https://uselessapi.com/u-c-r" + routes.route}>
                    {" "}
